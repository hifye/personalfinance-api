...
public sealed class PatchRecurringTransactionCommandHandler(
    ...
    ILogger<PatchRecurringTransactionCommandHandler> logger)
    : IRequestHandler<PatchRecurringTransactionCommand, Result>
{
    public async Task<Result> Handle(PatchRecurringTransactionCommand command, CancellationToken cancellationToken)
    {
        var recurringTransaction =
            await recurringTransactionRepository.GetRecurringTransactionById(command.Id, currentUser.UserId);
        ...
        var result = recurringTransaction.Patch(command.Amount, command.Type, command.Description, command.Frequency,
            command.IsActive);
        if (result.IsFailure)
            return result;
